...
class FilterValuesService {
  ...
  Future<Either<RewildError, List<LemmaByFilterId>>> getLemmasBySubjectId({
    ...
    if (localDbValues.isNotEmpty) {
      print(
          'Fetched lemmas (getLemmasBySubjectId) by subject ID from local database. ${localDbValues.length}');
    ...
    print(
        'Fetched lemmas (getLemmasBySubjectId) by subject ID from server. ${filterValues.length}');
    return right(filterValues);
  }
  ...
  Future<Either<RewildError, List<KwByLemma>>> getKeywordsByLemmas({
    ...
    for (final lemmaID in lemmasIDs) {
      ...
      if (localDbValues.isNotEmpty) {
        print(
            'Fetched keywords(getKeywordsByLemmas) from local database. ${localDbValues.length}');
    ...
    print(
        'Fetched keywords(getKeywordsByLemmas) from server. ${fetchedLemmas.length} and ${kwByLemmasFromLocalDb.length}');
  ...
  Future<Either<RewildError, List<KwByLemma>>> getKeywordsByWords({
    ...
    for (final word in words) {
      ...
      if (localDbValues.isNotEmpty) {
        print(
            'Fetched keywords(getKeywordsByWords) from local database. ${localDbValues.length}');
        kwByLemmasFromLocalDb.addAll(localDbValues);
      } else {
    ...
    print(
        'Fetched keywords(getKeywordsByWords) from server. ${fetchedLemmas.length} and ${kwByLemmasFromLocalDb.length}');
    return right(kwByLemmasFromLocalDb + fetchedLemmas);
  }
}
